...
async function content() {
  ...
  window.searchUserHandler = async function () {
    ...
    const resFoundUserData = await fetch(
      'http://localhost:8080/api/searchUserName',
      {
        method: 'POST',
    ...
    if (resFoundUserData.status === 400) {
      ...
      foundUserData.map(async (foundUser) => {
        ...
        const tweets = await fetch(
          `http://localhost:8080/api/tweet/${foundUser._id}`,
          {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
          }
        ...
        const favoriteTeam = await fetch(
          `http://localhost:8080/api/team/${foundUser._id}`,
          {
            method: 'Get',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
  ...
  window.searchTWEETHandler = async function () {
    ...
    const resFoundTweetData = await fetch(
      'http://localhost:8080/api/searchTweetContent',
      {
        method: 'POST',
  ...
  window.commentHandler = async function (index, tweetId) {
    ...
    // comment on tweet
    const up = await fetch('http://localhost:8080/api/comment', {
      method: 'POST',
      body: JSON.stringify({
        tweetId,
  ...
  window.tweetLikeHandler = async function (button) {
    ...
    // like tweet
    const up = await fetch('http://localhost:8080/api/like', {
      method: 'POST',
      body: JSON.stringify({
        tweetId,
        userId: user.userId,
  ...
  window.tweetDeleteHandler = async function (button) {
    const tweetId = button.getAttribute('data-tweet-id');
    // like tweet
    const up = await fetch(`http://localhost:8080/api/tweet/${tweetId}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
    });
    const data = await up.json();
  ...
  window.commentLikeHandler = async function (button) {
    ...
    // like tweet
    const up = await fetch('http://localhost:8080/api/likeComment', {
      method: 'POST',
      body: JSON.stringify({
        tweetId,
        commentIndex,
  ...
  window.commentDeleteHandler = async function (button) {
    ...
    const commentIndex = button.getAttribute('data-comment-id');
    // like tweet
    const up = await fetch('http://localhost:8080/api/deleteComment', {
      method: 'POST',
      body: JSON.stringify({
        tweetId,
        commentIndex,
